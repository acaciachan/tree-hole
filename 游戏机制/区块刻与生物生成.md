                ...
                那么：执行此区块此生物类别的生成SpawnCategoryForChunk # 此区块此生物类别的生成
                    ...
                    # 生成生物主代码：
                        如果这个生成原点O的方块不是红石导体：# 非常重要，刷怪平面内不能有“可导方块”
                            clusterSize = 0 # 三组游走的总的生成数量计数器
                            三组成群生成：
                                游走起点重新设置为生成原点O
                                count = 随机1~4 # 仅作为初始默认数值，后面会根据选中的生物修改此数值
                                groupSize = 0 # 热带鱼相关的计数器                           
                                每组成群生成(count次)：
                                    ...
                                    那么：
                                        ...
                                        count = spawnerData这种生物的生成随机次数(最少次数~最大次数) # 常见生物多为4，罕见生物多为1
                                        ...
                                        那么：
                                            ...
                                            那么：
                                                ...
                                                如果 isMaxGroupSizeReached(groupSize)，那么终止“当前这组成群生成”，继续下一组成群生成
...
某种生物和当前群系的组合，在群系json文件里可能存在的生成势能和生成预算，这种情况下就会进行势能相关的判断
    ...
    计算某目标生物x在某个位置P的势能，以及能否生成：
        E = charge_x * ∑(维度势能列表所有生物：i， charge_i / distance_x~i)
...
2.  由于是“先游走，再选怪，后生成”，但是这部分的逻辑为：
        三次游走：
            ...
            每次游走：
                水平随机偏移
                如果 距离玩家 > 24：
                    未选择生物则尝试选择生物(并把count设置为此生物的成群生成数量)
                    尝试刷怪...
    
    可以看到，选怪只有离开半径24的范围才能选择生成的生物以及后续刷怪，否则直接进行下一次游走；
    可以把半径24的边界放在女巫小屋结构的边上，这样子可以避免在女巫小屋的外面就选怪(选中女巫概率很低)，
    只有游走进入女巫小屋才会选怪(100%选择女巫)，以提高游走的有效率。
    可以采用4个或更多玩家的半径24区域“挤压”刷怪平台的方式提高效率，
    但是根据上述，仅适用于刷怪平台的位置此种生物的选中概率明显大于区域外，或刷怪平台比较小效果提升才会明显
    目前仅适用于女巫塔，特别是例如“1.18+种子：-3767261087998042360 /tp 512 ~ 512”，这种低y女巫小屋


3.  mobsAt获取此位置生成生物的列表时，优先使用结构的生物列表(直接覆盖群系的生物列表)
    没有结构(或结构的生物列表为空)再使用生物群系的。

4.  三组成群生成，每组成群生成的每一个个体都要判断：
    4.1 距离玩家d需要满足 24 < d <= 128 
        or (不会远离玩家消失(!removeWhenFarAway) and 能远离玩家生成(canSpawnFarFromPlayer))
    4.2 每个生成位置的结构和群系的列表里依然有这种生物
    4.3 此处没有方块碰撞箱阻挡
    4.4 生成势能(主要是灵魂沙峡谷某些生物)
    4.5 通过四个复杂检测：
        4.4.1   此生物的SpawnPlacements.Types
        ...
        4.4.2   此生物的SpawnPlacements.checkSpawnRules # 每个生物的特色判断
        
        4.4.3   Mob.checkSpawnRules # “12亮度”的大小判断
            生物(Mob)：true
                寻路类生物(PathFinderMob)：true(总是可以生成)
                    动物：下方是草方块 or 实际光照 >= 12
                        蘑菇牛：下方是菌丝 or 实际光照 >= 12
        ...
        4.4.4   Mob.checkSpawnObstruction # 无实体碰撞箱阻挡，某些生物液体阻挡判断
            生物(Mob)两个判断：
